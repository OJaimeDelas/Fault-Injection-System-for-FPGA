# =============================================================================
# FATORI-V • FI Profiles Time
# File: input.py
# -----------------------------------------------------------------------------
# Forwarder for loading external time profiles from custom Python modules.
#=============================================================================
...
def describe() -> str:
    ...
    """
    return "Load external time profile from custom Python module"
...
def make_profile(args: Dict[str, Any], *, global_seed, settings):
    """
    Load and validate external time profile module.
    
    This forwarder profile allows users to provide custom time profiles
    ...
    Returns:
        Profile instance from external module (must have run method)
    
    Raises:
        ValueError: If module_path missing, file not found, invalid interface
    
    Example:
        >>> # User provides custom profile at /tmp/my_time.py
        >>> args = {"module_path": "/tmp/my_time.py", "delay": 0.5}
    ...
    if not module_path:
        raise ValueError(
            ...
            "Usage: --time input --time-args 'module_path=/path/to/custom.py'"
        )
    
    # Load the external Python module from file system
    external_module = _load_external_module(module_path)
    
    # Validate module implements required time profile interface
    _validate_time_profile_module(external_module, module_path)
...
def _validate_time_profile_module(module, module_path: str):
    """
    Validate external module implements required time profile interface.
    
    Time profiles must provide:
    - PROFILE_KIND = "time"
    - PROFILE_NAME (string)
    - describe() function
    ...
    if not hasattr(module, "PROFILE_KIND"):
        raise ValueError(
            ...
            f"Add: PROFILE_KIND = 'time'"
        )
    
    # Check PROFILE_KIND has correct value
    if module.PROFILE_KIND != "time":
        raise ValueError(
            f"Module has PROFILE_KIND='{module.PROFILE_KIND}' "
            f"but expected 'time' for time profile: {module_path}"
        )
    ...
    if missing:
        raise ValueError(
            ...
            f"Time profiles must define: {', '.join(required)}"
        )
    ...
    # make_profile() and any issues will surface when run() is called.